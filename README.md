...
### New York City Borough's Neighborhoods
...
3. Remove rows containing invalid values in any column. 
...
2. Count the total occurrences of each property type.
...
2. Count the total number of unique host identification numbers per property type.
...
2. Count the total occurrences of each property type in each borough.
...
2. Calculate the total occurrences of each property type in each neighborhood of each borough, and sort the results from highest to lowest.
...
3. Plot a line graph of the top five neighborhoods in each borough with the lowest average minimum night requirements.
